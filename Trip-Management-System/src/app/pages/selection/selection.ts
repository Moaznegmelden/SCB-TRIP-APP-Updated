...
export class Selection implements OnInit {
  ...
  loadApplicants(): void {
    ...
    const requestId =
      ++this.applicantsRequestId;

    this.loadingApplicants =
      true;
    ...
    if (this.selectedTripId === null) {
      ...
      url =
        `${this.apiUrl}/selections/applicants`;

    // ONE TRIP - ALL BATCHES
    } else if (
      this.selectedBatchId === null
    ) {

      url =
        `${this.apiUrl}/selections/trip/${this.selectedTripId}/applicants`;

    // ONE TRIP - ONE BATCH
    } else {

      url =
        `${this.apiUrl}/selections/trip/${this.selectedTripId}/batch/${this.selectedBatchId}/applicants`;
    }
    ...
    this.http
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          // HTTP 204 means this batch has no request yet.
          if (error?.status === 204) {
            ...
            console.error(
              'Failed to load batch selection request:',
              error
            );
  ...
  get selectedTripBatches():
    TripBatch[] {

    if (
      !this.selectedTrip?.batches
    ) {
      return [];
    }
  ...
  sendToManager(): void {
    ...
    if (!this.isSpecificBatchSelected) {
      ...
      this.errorMessage =
        'Please select one specific batch before sending the selection method to the manager.';
    ...
    if (
      ...
      this.errorMessage =
        'Please select both a trip and a batch first.';
    ...
    if (
      ...
      this.errorMessage =
        'There are no applicants for the selected batch.';
    ...
    if (!hrUserId) {
      ...
      this.errorMessage =
        'No logged-in HR Admin was found. Please log in again.';
    ...
    this.http
      ...
      .subscribe({
        ...
        next: (response) => {
          ...
          this.successMessage =
            'Selection method was sent successfully to the HR Manager and is waiting for approval.';
        ...
        error: (error) => {
          ...
          console.error(
            'Failed to submit selection request:',
            error
          );
